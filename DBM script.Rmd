...
###Darjeeling 2
   ...
   guides(color = guide_legend(keywidth = 2, keyheight = 1.8,override.aes = list(shape = NA)))
...
tiff("Figure3.tiff", height = 4.09, width = 7.4, units = 'in', 
...
tiff("Figure5.tiff", height = 4.09, width = 7.4, units = 'in', 
     compression = "lzw", res = 1200)
Figure5
dev.off()
...
expression=read.csv("Raw_qPCR.csv")

p <- expression %>%
  mutate(life_stage = fct_relevel(life_stage, 
            "L2","L4","Pupae","3DPE"))
plot <- ggplot(aes(x = life_stage, y = relative_quantification_values), data=p)+ 
geom_sina(stat="sina", position="dodge", kernel="gaussian", method="density", aes())+theme(legend.position = "none")+
  stat_summary(fun.y = median, fun.ymin = median, fun.ymax = median,geom = "crossbar", width = 0.5,size=0.2)+
  theme(legend.position="none")+
  ...
  theme(axis.line.x = element_line(color="black", size = 1),axis.line.y = element_line(color="black", size = 1))+
  labs(y="Normalized Relative Quantity \n (NRQ)", x="", size=0.5)+
  annotate("text", size=2, label = "a", x = 1, y = 2.5)+
  annotate("text", size=2, label = "ab", x = 2, y = 2.5)+
  annotate("text", size=2, label = "ab", x = 3, y = 2.5)+
  annotate("text", size=2, label = "bc", x = 4, y = 2.5)+
theme_classic(base_size=6)
  
tiff("Figure4.tiff", height = 2, width = 3.5, units = 'in', 
     compression = "lzw", res = 1200)
plot
dev.off()

m1 <- lm(rank(relative_quantification_values)~life_stage, data=p)
m2 <- glht(m1, linfct=mcp(life_stage="Tukey"))

library(sjPlot)
tab_model(m1, file="table.html")

```

```{r}
geom_data <- read_csv("GEOM_MEAN.csv")
...
=======
>>>>>>> db57f2cf198bbcd1565f3895c4a002ca26d57158
...
<<<<<<< HEAD
 ...
 scale_fill_manual(values=pal)+
  ...
  annotate("text", size=2, label = "bc", x = 2.22, y = 0.7)+
    annotate("text", size=2, label = "a", x = 2.75, y = 0.29)+
  annotate("text", size=2, label = "abc", x = 3.22, y = 0.72)+
          annotate("text", size=2, label = "b", x = 3.75, y = 0.57)+
  annotate("text", size=2, label = "cd", x = 4.22, y = 1.29)+
                annotate("text", size=2, label = "ab", x = 4.75, y = 0.55)+
  annotate("text", size=2, label = "de", x = 5.22, y = 1.96)+
                 annotate("text", size=2, label = "c", x = 5.75, y = 0.84)+
  annotate("text", size=2, label = "e", x = 6.22, y = 2.48)+
                   annotate("text", size=2, label = "ab", x = 6.75, y = 0.57)+
  annotate("text", size=2, label = "cd", x = 7.22, y = 0.92)

```

```{r}
tiff("alternative_Figure5.tiff", height = 3.5, width = 3.5, units = 'in', 
     compression = "lzw", res = 1200)
p
dev.off()
=======
